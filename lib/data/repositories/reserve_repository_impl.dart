...
class ReserveRepositoryImpl implements ReserveRepository {
  ...
  Future<Either<Failure, List<ReserveEntity>>> getAllReserves() async {
    try {
      ...
      return const Left(ServerFailure('Error al cargar reservas'));
    }
  }
  ...
  Future<Either<Failure, bool>> deleteReserve(int idReserve) async {
    try {
      final token = sharedPreferences.getString('token');
      await remoteDataSource.deleteReserves(idReserve, token!);
      return const Right(true);
    } catch (e) {
      return const Left(ServerFailure('Error al eliminar el reserva'));
    }
  }
  ...
  Future<Either<Failure, bool>> updateReserve(ReserveEntity reserve) async {
    try {
      ...
      return Left(
        ServerFailure('Error al actualizar la reserva: ${e.toString()}'),
      );
    }
  }
  ...
  Future<Either<Failure, bool>> addUserToReserve(
    ...
    try {
      ...
      return Left(
        ServerFailure(
          'Error al añadir jugador a la reserva: ${e.toString()}',
        ),
      );
    }
  }
  ...
  Future<Either<Failure, bool>> deleteUserOfReserve(
    ...
    try {
      ...
      return Left(
        ServerFailure(
          'Error al eliminar jugador de la reserva: ${e.toString()}',
        ),
      );
    }
  }
  ...
  Future<Either<Failure, List<ReserveEntity>>> getAllReservesByDate(
    ...
    try {
      ...
      final reserveModels = await remoteDataSource.getAllReservesByDate(
        formattedDate,
        token!,
        tableId,
      );
      ...
      return Left(
        ServerFailure('Error al cargar reservas por fecha: ${e.toString()}'),
      );
    }
  }
  ...
  /// Throws an [Exception] if there is an error during the retrieval process.
  Future<Either<Failure, ReserveEntity>> getReserveWithUsers(
    int idReserve,
  ) async {
    try {
      final token = sharedPreferences.getString('token');
      final reserveModels =
          await remoteDataSource.getReserveById(idReserve, token!);
      List<Future<dynamic>> userAvatars =
          reserveModels.users!.map((user) async {
      ...
      ReserveEntity reserveEntity =
          reserveModels.toReserveEntityWithUsers(avatars);
      return Right(reserveEntity);
    } catch (e) {
      return const Left(ServerFailure('Error al cargar reserve'));
    }
  }
  ...
  Future<Either<Failure, List<ReserveEntity>>> getReservesOfUser(
    ...
    try {
      final token = sharedPreferences.getString('token');
      final reserveModels =
          await remoteDataSource.getReservesOfUser(userId, token!);
      List<ReserveEntity> reserveEntities =
          reserveModels.asMap().entries.map((entry) {
        return entry.value.toReserveEntity();
      }).toList();
      ...
      return Left(
        ServerFailure(
          'Error al cargar reservas del usuario: ${e.toString()}',
        ),
      );
    }
  }
  ...
  Future<Either<Failure, bool>> confirmReserve(int idReserve) async {
    try {
      final token = sharedPreferences.getString('token');
      ...
      return Left(
        ServerFailure('Error al confirmar la reserva: ${e.toString()}'),
      );
    }
  }
  ...
  Future<Either<Failure, List<int>>> createMultipleReservesEvent(
    ...
    try {
      ...
      return Left(
        ServerFailure(
          'Error al crear múltiples reservas de evento: ${e.toString()}',
        ),
      );
    }
  }
  ...
  Future<Either<Failure, List<ReserveEntity>>> getEvents(int idShop) async {
    try {
      final token = sharedPreferences.getString('token');
  ...
  Future<Either<Failure, List<UserEntity>>> getLastTenPlayers(
    String idUser,
  ) async {
    try {
      final token = sharedPreferences.getString('token');
      ...
      return Left(
        ServerFailure(
          'Error al obtener los últimos diez jugadores: ${e.toString()}',
        ),
      );
    }
  }
